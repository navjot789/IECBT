...
if(isset($_POST['username'],$_POST['password'],$_POST['password_encryption']))
{
   ...
   $password_encryption= mysqli_real_escape_string($dbc,$_POST['password_encryption']);
	...
	if($username =='' || $password=='')
		{
              ...
              if($password_encryption == 'MD5')
              {
					...
					$stmt = $dbc->prepare("SELECT aid,username,password FROM admin_login WHERE username= ? AND password = ?");
					...
					{
						$stmt = $dbc->prepare("SELECT aid,username, AES_DECRYPT(password,'".PASSWORDS_ENCRYPT_KEY."') FROM admin_login WHERE username= ? AND password= AES_ENCRYPT(?,'".PASSWORDS_ENCRYPT_KEY."')");
